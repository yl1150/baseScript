cc.Class({
    ...
    onLoad() {
        ...
        this.qiu = this.base.getChildByName("qiu");
        this.qiu_arr = this.qiu.children.map(x => { return x });
        this.qiu_arr.forEach(qiu => {
            qiu.active = false;
        })
        ...
        this.sbSke_new = this.item_arr[3].getComponent(sp.Skeleton);
        this.item_arr[3].active = false;

        this.dbAction = this.base.getChildByName("dbAction");
        this.dbSke = this.dbAction.getComponent(sp.Skeleton);

        this.dbAction2 = this.base.getChildByName("dbAction2");
        this.dbSke_new = this.dbAction2.getComponent(sp.Skeleton);

        this.dbAction.active = false;
        this.dbAction2.active = false;

        this.btnImg = this.base.getChildByName("btnImg");
        this.curImg = this.base.getChildByName("curImg");
        this.curImg.active = false;
    ...
    touchSubmitEnd(event) {
        ...
        //合上挡板
        var curType = this.btnImg._curType;
        if (curType == 1) {
            ...
            cc.origin.ScheduleBase.addTimeout(() => {
                cc.origin.AudioBase.play("sb_ans", () => { this.sceneGameStar() });
            }, 2);
        } else if (curType == 2) {
            //合上挡板
            this.dbAction2.active = true;
            this.dbSke_new.setAnimation(0, 'newAnimation_2', false);
            this.dbSke_new.setCompleteListener(() => {
                this.qiu_arr2.forEach(qiu => {
                    qiu.active = false;
                    qiu.getComponent(sp.Skeleton).setAnimation(0, 'newAnimation', false);
                })
                ...
                this.jiqiSke.setCompleteListener(() => {
                    //数宝抱着糖豆
                    if (!isOnce) {
                        this.sbSke_new.setSkin("tang1")
                        this.sbSke_new.setAnimation(0, 'newAnimation_5', false);
                        this.sbSke_new.setCompleteListener(() => {
                            this.sbSke_new.setAnimation(0, 'newAnimation_4', true);
                        });
                        isOnce = true;
                    }
                })
    ...
    showGameStar() {
        ...
        cc.origin.AudioBase.play("title_tip", () => {
            this.setQiuAction();
            cc.origin.AudioBase.play("title_ans", () => {
                cc.origin.ScheduleBase.addTimeout(() => {
                    this.ddSke.setAnimation(0, 'newAnimation_1', true);
                    this.btnImgTouch(1);
                }, 1)
            });
            this.ddSke.setSkin("blue")
    ...
    setQiuAction() {
        ...
        for (let k in delay_arr) {
            ...
            var upWord = this.qiu_arr[k].convertToWorldSpaceAR(cc.v2(0, 0));
            var upPos = this.qiu.convertToNodeSpaceAR(upWord);
            this.qiu_arr[k].setParent(this.qiu);
            this.qiu_arr[k].setPosition(upPos);
            cc.origin.ScheduleBase.addTimeout(() => {
                this.qiu_arr[k].active = true;
                if (k != 7) {
                    this.qiu_arr[k].getComponent(sp.Skeleton).setAnimation(0, 'newAnimation_1', false);
                }
                cc.tween(this.qiu_arr[k])
    ...
    setQiuAction2() {
        ...
        for (let k in delay_arr) {
            ...
            var upPos = this.qiu.convertToNodeSpaceAR(upWord);
            this.qiu_arr2[k].setParent(this.qiu);
            this.qiu_arr2[k].setPosition(upPos);
            cc.origin.ScheduleBase.addTimeout(() => {
                this.qiu_arr2[k].active = true;
                // if (k != 7) {
                //     this.qiu_arr2[k].getComponent(sp.Skeleton).setAnimation(0, 'newAnimation_1', false);
                // }
    ...
    sceneGameStar() {
        ...
        cc.origin.Note.trumpet.setTrumpet('round' + this.roundId, false, () => {
            this.dbAction2.active = true;
            this.item_arr[2].active = false;
            this.item_arr[3].active = true;
            this.sbSke_new.setAnimation(0, 'zheng1', true);
            this.dbSke_new.setAnimation(0, 'newAnimation_1', false);
            this.dbSke_new.setCompleteListener(() => {
                this.dbAction2.active = false;
            })
            var isFirst = false;
            this.ddSke.setAnimation(0, 'newAnimation_3', true);
            this.ddSke.setCompleteListener(() => {
                if (isFirst) {
    ...
    showResult(isWin = true) {
        ...
        function simulate() {
            //星星、填入数值
            cc.origin.AudioBase.play('star');
            cc.origin.Script.Tools.addStarEffect(self.option_arr[self.answer - 1]);
            cc.origin.EventManager.emit(cc.origin.EventDefine.COMMON.KINETIC_TIME)
            //演示
            cc.origin.ScheduleBase.addTimeout(hint, 1);
        }
        function hint() {
            //伴随语音提示
            var isOnceT = false
            self.ddSke.setSkin("red");
            self.ddSke.setAnimation(0, 'newAnimation_3', false);
            self.ddSke.setCompleteListener(() => {
                if (!isOnceT) {
                    self.setOverAction();
    ...
    setOverAction() {
        ...
        var upPos = this.qiu.convertToNodeSpaceAR(upWord);
        this.qiu_arr2[index].setParent(this.qiu);
