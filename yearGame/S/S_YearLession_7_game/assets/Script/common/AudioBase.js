...
var AudioBase = {
    ...
    preloadAudioClip() {
        ...
        if (cc.gameConfig && cc.gameConfig.soundAssets) {
            //音频已加载
            cc.gameConfig.soundAssets.forEach((pClip) => {
                this.album[pClip.name] = pClip;
            })
        } else {
            cc.log('音频未加载 手动加载');
            cc.assetManager.loadBundle('soundBundle', (err, bundle) => {
                bundle.loadDir("", cc.AudioClip, (err, clip_arr) => {
                    clip_arr.forEach((pClip) => {
                        this.album[pClip.name] = pClip;
                    })
                });
    ...
    loadVoice(soundNameStr, callback) {
        //由于传入的str 可能带有路径 切除str
        if (this.album[soundNameStr]) {
            console.log('查找到对应音频')
            return this.album[soundNameStr];
        } else {
            cc.assetManager.loadBundle('soundBundle', (err, bundle) => {
                bundle.load(soundNameStr, cc.AudioClip, (err, clip) => {
