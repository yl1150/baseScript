cc.Class({
    ...
    showWin(spineId = 0, cb) {
        ...
        if (spineId === 1) {
            animationName = "newAnimation2";
        } else if (spineId === 2) {
            animationName = "animation4";
        } else if (spineId === 3) {
            animationName = "newAnimation";
        }
        skeCom.setAnimation(0, animationName, false);
        setTimeout(() => {
            ske.active = false;
    ...
    // /**
    // * 显示胜利动画
    // * @param {Vec2} starWorldpos 星星位置
    // * @param {Number} spineId 动画id
    // * @param {Function} cb 结束回调
    // */
    // showWin(starWorldpos, spineId = 0, cb) {
    //     var count = spineId;
    //     this.winSpines.active = true;
    //     this.grey.active = true;
    //     //位置
    ...
    //     }
    //     skeCom.setAnimation(0, animationName, false);
    //     skeCom.setEventListener(() => {
    //         this.earnStar(starWorldpos, count);
    //     });
    //     setTimeout(() => {
    //         ske.active = false;
    //         this.grey.active = false;
