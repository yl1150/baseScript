...
cc.Class({
    ...
    initAnswer(data, type = 'add') {
        if (type == 'add') {
            this.addRightAnswers[0].cells = [data[0], data[1], data[2]];
            this.addRightAnswers[1].cells = [data[1], data[0], data[2]];
        } else {
            this.subRightAnswers[0].cells = [data[2], data[1], data[0]];
            this.subRightAnswers[1].cells = [data[2], data[0], data[1]];
        }
        console.log(this.addRightAnswers)
        console.log(this.subRightAnswers)
    ...
    init(rightCB, wrongCB) {
        ...
        this.formulaList.children.forEach((formula) => {
            ...
            formula.children.forEach((option) => {
                ...
                baseKey && baseKey.init(this.touchKey.bind(this));
    ...
    touchkeyBoard() {
        ...
        this.formulaList.children.forEach((formula) => {
            formula.children.forEach((option) => {
                if (option.baseKey && option.baseKey.isUsed && !option.baseKey.getKeysState()) isAllLock = false;
            });
        });
    ...
    checkIsAllRight() {
        ...
        this.formulaList.children.forEach((formula) => {
            let keyArr = []
            formula.children.forEach((option) => {
                option.baseKey && option.baseKey.isUsed && keyArr.push(option.baseKey.getKeys());
            });
