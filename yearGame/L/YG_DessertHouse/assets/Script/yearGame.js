cc.Class({
    ...
    registerEvent() {
        //继续下一环节游戏
        cc.YL.emitter.on('continueGame', (data) => {
            console.log('continueGame')
            this.continueGame();
        })

        cc.YL.emitter.on('showGetGift', (name) => {
            console.log('showGetGift')
            this.showGetGift(name);
        })
    },
    ...
    showGetGift(name) {
        ...
        let getGiftNode = this.node.getChildByName('getGift');
        let gift = getGiftNode.getChildByName(name);
        GD.root.showStar(gift);
        getGiftNode.zIndex = 99;
        getGiftNode.active = true;
        gift.active = true;
        cc.YL.timeOut(() => {
            gift.active = false;
            getGiftNode.active = false;
        }, 2000)
    },
