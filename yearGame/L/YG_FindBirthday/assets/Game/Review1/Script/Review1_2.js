...
cc.Class({
    ...
    startPlay() {
        ...
        this.key_arr.forEach(key => {
            ...
            skeCom.setAnimation(0, 'newAnimation', true);
        })
        //题目语音且可操作
        ...
        //初始化选项
        this.option_arr = this.week_arr.map(x => { return x });
        //给选项注册触摸
    ...
    checkResult(option) {
        ...
        var weekId = this.getWeekId(option);
        var isCorrect = (monthId === this.answer.monthId && weekId === this.answer.weekId);
        if (isCorrect) {
    ...
    showResult(isWin = true) {
        ...
        function simulate() {
            ...
            cc.origin.ScheduleBase.addTimeout(() => {
                //正确日期星星
                var weekId = self.answer.weekId;
                var correctOption = self.week_arr[weekId];
        ...
        function hint() {
            ...
            cc.origin.ScheduleBase.addTimeout(() => {
                ...
                cc.origin.AudioBase.play('bubble');
                var weekId = self.answer.weekId;
                var week = self.week_arr[weekId];
                var chosen_week = week.getChildByName('chosen');
                chosen_week.active = true;
                cc.tween(chosen_week).to(0.5, { scale: 1.2 }).to(0.5, { scale: 1 }).start();
            }, 2.5);
        }
    },
});